...
export const AIVertigoRehabCoach: React.FC = () => {
  ...
  const addManualRep = useCallback(() => {
    const target = exerciseRef.current.targetAngle || 20;
    const newReps = repRef.current.reps + 1;
    repRef.current = {
      ...repRef.current,
      reps: newReps,
      peaks: [...repRef.current.peaks, target],
      velocities: [...repRef.current.velocities, 120],
    };
    setSavedSessionId(null);
    setHud((prev) => ({
      ...prev,
      reps: newReps,
      peaks: [...prev.peaks, target],
      velocities: [...prev.velocities, 120],
    }));
  }, []);
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (face) {
        ...
        // 3. Count, but never while calibrating — only for drills with a tracked head axis.
        const currentAxis = exerciseRef.current.axis;
        if (!calibrationSamplesRef.current && currentAxis !== null) {
          const ex = exerciseRef.current;
          const trackedAxis: HeadAxis = currentAxis;
          ...
          <div className="bg-white dark:bg-ink-900 rounded-2xl border border-slate-200 dark:border-ink-800 p-5 shadow-elevated space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl border border-emerald-300 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/50 text-emerald-800 dark:text-emerald-200 font-bold text-xs disabled:opacity-50 transition-colors hover:bg-emerald-100 dark:hover:bg-emerald-900/60"
              >
                +1 {hi ? 'रेप' : 'Rep'}
              </button>
            ...
            <div className="space-y-3">
              ...
              {/* Live axis meter — only for head-angle tracked drills */}
              {exercise.axis !== null ? (
                <div>
                  <div className="flex items-center justify-between text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">
                ...
                <div className="p-2.5 rounded-xl border border-slate-200 dark:border-ink-800 bg-slate-50 dark:bg-ink-950 text-slate-600 dark:text-slate-400 text-xs flex items-center justify-between">
                  <span>{hi ? 'गाइडेड अभ्यास — प्रत्येक मूवमेंट के बाद +1 रेप टैप करें' : 'Guided exercise — tap "+1 Rep" after completing each movement'}</span>
                  <span className="font-semibold text-teal-600 dark:text-teal-400">{hi ? 'कैमरा सहायता सक्रिय' : 'Camera visual guide active'}</span>
                </div>
              )}
            </div>
